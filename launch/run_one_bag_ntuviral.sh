...
# Get playback rate from launch file (default 0.2 if not specified)
PLAYBACK_RATE=${12:-0.2}
# Adjust LOG_DUR based on playback rate: if rate is 0.2, duration is 5x longer
LOG_DUR=$(awk "BEGIN {printf \"%.0f\", $BAG_DUR/$PLAYBACK_RATE+20}")

echo "BAG DURATION:" $BAG_DUR "=> PLAYBACK_RATE:" $PLAYBACK_RATE "=> LOG_DUR:" $LOG_DUR;
